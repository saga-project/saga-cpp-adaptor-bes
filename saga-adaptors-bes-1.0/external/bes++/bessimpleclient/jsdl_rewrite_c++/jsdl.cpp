...
struct new_jsdl_range
new_jsdl_processRange (struct soap_dom_element * dom)
{
    // struct jsdl_range *r;
    // struct soap_dom_element *cur;
    // struct soap_dom_attribute *attr;
  ...
  ret.lowerBound.value = strtod (cur->data, NULL);
  ret.lowerBound.exclusive = 0;
  ...
  ret.upperBound.value = strtod (cur->data, NULL);
  ret.upperBound.exclusive = 0;
...
new_jsdl_range_value
new_jsdl_processRangeValue (struct soap_dom_element *dom)
{
  ...
  struct soap_dom_element     * cur;
  struct soap_dom_attribute   * attr;
  struct new_jsdl_range_value   ret;
  ...
  while ( cur )
  {
    if ( isElement (cur, JSDL_NS, "UpperBoundedRange") )
    {
      ret.upperBoundedRange.value = strtod (cur->data, NULL);
      ...
      for ( attr = cur->atts; attr; attr = attr->next )
      {
        if ( ! strcmp (attr->name, "exclusiveBound") )
        {
          if ( ! strcmp (attr->data, "true") )
          {
            ret.upperBoundedRange.exclusive = 1;
          }
        }
      }
    }
    ...
    {
      ret.lowerBoundedRange.value = strtod (cur->data, NULL);
      ...
      for ( attr = cur->atts; attr; attr = attr->next )
      {
        if ( ! strcmp (attr->name, "exclusiveBound") )
        {
          if ( ! strcmp (attr->data, "true") )
          {
            ret.lowerBoundedRange.exclusive = 1;
          }
        }
      }
    }
    ...
    {
      ret.exact.value   = strtod (cur->data, NULL);
      ret.exact.epsilon =  0.0;

      for ( attr = cur->atts; attr; attr = attr->next )
      {
        if ( ! strcmp (attr->name, "epsilon") )
        {
          ret.exact.epsilon = strtod (attr->data, NULL);
          break;
        }
      }
    }
    ...
    {
      ret.range = new_jsdl_processRange (cur);
    }
                      ...
                      struct jsdl_job_definition *jsdl)
{
    ...
    while (cur) {
        if (isElement(cur, JSDL_NS, "CandidateHosts")) {
            ...
            jsdl->TotalCPUCount = new_jsdl_processRangeValue (cur);
        } 
...
struct soap_dom_element *
jsdl_generateRangeValue (struct soap *s, const struct new_jsdl_range_value & rangeval, char *name)
{
    struct soap_dom_element *rval, *first, *cur, *dom;
    struct jsdl_exact *exact;
    struct jsdl_range *range;
    ...
    if ( rval == NULL ) 
    {
      throw "Cannot create dom";
    }

    first = cur = NULL;

    if ( rangeval.UpperBoundedRange.set ) 
    {
        dom = jsdl_generateDomElement(s, JSDL_NS, "UpperBoundedRange");
        if (dom == NULL) {
            return NULL;
...
jsdl_generateResources(struct soap *s, struct jsdl_job_definition *jd)
{
    ...
    dom = new_jsdl_generateRangeValue (s, jd->TotalCPUCount, "TotalCPUCount");
    if ( dom != NULL )
    {
      dom->prnt = res;
      if ( ! first )
